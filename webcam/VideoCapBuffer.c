...
int frameBufferNumber = 0;
unsigned int bufferFrameSeqNum = 0;
double t_last = 0.0;
...
static void process_image3(const void* p,int len)
{
  ...
  process_image                   (const void *           p)
{
  ...
  if(currentAVIFile == NULL) {
    char fileName[] = "/home/gary/testvid/vid/videoj0000.avi";
    char str[256];
    memset((void*)str,'0',256);
    sprintf(str,"%d",fileNumber);
    if(fileNumber < 10)
  ...
  read_frame                      (void)
{
  ...
  case IO_METHOD_MMAP:
    ...
    *		first_pts_timestamp = buf.timecode.type;
    ...
    if(t_last > 0) {
      frame_rate = 1.0 / ((t2 - t_last)*t_period);
      //printf("frame rate: %f\n",frame_rate);
      //printf("PTS: %u\n",buf.timecode.type);
    }
    ...
    memcpy(file_name,video_working_directory,sizeof(file_name));
    memcpy(file_name_buffer,video_working_directory,sizeof(file_name));
    int file_name_base_len = strlen(file_name);
    
    sprintf(file_name_buffer+file_name_base_len,"frame.seq");

    
    FILE* buffer_file = NULL;
    FILE* buffer_seq_file = NULL;

    double meanTime = meanFrameTime;
    double framesPerSec = 0.0;

    if(framesInFile > 0 && meanTime > 0.0)
    {
      meanTime /= (double)framesInFile;
      framesPerSec = 1.0 / meanTime;
    }
    ...
    for(i = 0; i < 10; i++)
    {
      sprintf(file_name+file_name_base_len,"buffer/frame%d.jpg",frameBufferNumber);
      buffer_file = fopen(file_name,"wb");

      if(buffer_file != NULL)
      {
        fwrite(buffers[buf.index].start,dataCount,1,buffer_file);
        fwrite(&bufferFrameSeqNum,sizeof(bufferFrameSeqNum),1,buffer_file);
        fwrite(&framesPerSec,sizeof(frame_rate),1,buffer_file);
        fclose(buffer_file);
        break;
      }
      ...
      //frameBufferNumber = frameBufferNumber % 300;
    }
    ...
    for(i = 0; i < 10; i++)
    {
      buffer_seq_file = fopen(file_name_buffer,"wb");

      if(buffer_seq_file != NULL)
      {
        fwrite(&frameBufferNumber,sizeof(frameBufferNumber),1,buffer_seq_file);
        time_t t = time(0);
        fwrite(&t,sizeof(t),1,buffer_seq_file);
        fclose(buffer_seq_file);
      }
    }
